...
2. Modify the project so that you can minimize or solve those redundancy.
	Answer : We can call constractor by using keyword for example in my project use constractor name shape color to reduce the redundant codes of construction in program.

3. What happen if you uncomment line #27-28 in the Program.cs file? Why?
	Answer :It'll be error because Shape is abstract class.We cann't use Shape class for instantiate objects. 
...
4. If you uncomment line #30-24 in the Program.cs file, you will get some compile errors.
   ...
   Answer : Add printPerimeter methods that program can use printPerimeter() for show  perimeter in MainClass but outside the Main method .
